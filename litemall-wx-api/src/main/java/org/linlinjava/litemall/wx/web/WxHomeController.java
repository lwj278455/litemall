...
public class WxHomeController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        Callable<List> bannerListCallable = () -> adService.queryIndex(1);
        ...
        if (order!=null){
            Integer[] orderIds=orderGoodsService.selectGoodsId(order);
            List<Integer> orders = Arrays.asList(orderIds);
            dealList = ()-> goodsService.findByGoodsByDeal(orders,SystemConfig.getNewLimit());
        }
        ...
        FutureTask<List> hotGoodsListTask = new FutureTask<>(hotGoodsListCall);
        FutureTask<List> dealListTask = new FutureTask<>(dealList);
