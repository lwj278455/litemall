...
public class LitemallGoodsService {
    Column[] columns = new Column[]{Column.id, Column.name, Column.brief,Column.soldnumber,Column.number,Column.picUrl,Column.ispromotion,Column.isblack,Column.isHot, Column.isNew, Column.counterPrice,Column.retailPrice,Column.retailPrice};
    @Resource
    ...
    public List<LitemallGoods> querySelective(Integer catId, Integer brandId, String keywords,String name, Boolean ispromotion, Boolean isblack,Boolean isHot, Boolean isNew, Integer offset, Integer limit, String sort, String order) {
        LitemallGoodsExample example = new LitemallGoodsExample();
        ...
        if (!StringUtils.isEmpty(isblack)) {
            criteria1.andIsBlackEqualTo(isblack);
            criteria2.andIsBlackEqualTo(isblack);
        }
        if (!StringUtils.isEmpty(ispromotion)) {
            criteria1.andIsBlackEqualTo(ispromotion);
            criteria2.andIsBlackEqualTo(ispromotion);
        }
     ...
     * @return
     */
    public  List<LitemallGoods> findByGoodsByDeal(List<Integer> id,Integer pageSize) {
        LitemallGoodsExample example = new LitemallGoodsExample();
        example.or().andIdIn(id).andIsOnSaleEqualTo(true).andDeletedEqualTo(false);

        PageHelper.startPage(0,pageSize);
        return goodsMapper.selectByDealSelective(example,columns);
    }
